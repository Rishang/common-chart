...
import FragmentConfigContainers from '../fragments/config-containers.mdx';
...
- name: some-volume         # string    | Name of the volume
  ...
  secret:                   # struct    | Secret reference (for Secret Volumes)
    secretName: my-secret   # string    | Name of the secret
```
...
ingress:                    # struct    | Ingress for component
  ...
  tlsClusterIssuer: cert-manager-letsencrypt # string   | Name of tls cluster issuer (letsencrypt, cert-manager)
  ingressClassName: nginx   # string    | Ingress class name
...
rollingUpdate:              # struct    | Rolling update configuration
  enabled: true             # boolean   | Enable/disable rolling updates (default: true)
  ...
  partition: 0              # int       | Only update pods with ordinal number greater than or equal this value (default: 0)
```
...
## `serviceaccount`
```yaml
serviceaccount: {}               # struct    | ServiceAccount to create (see serviceaccount.mdx)
```

## `configmap`
```yaml
configmap: {}                    # struct    | ConfigMap to create (see configmap.mdx)
```

## `secrets`
```yaml
secrets: {}                      # struct    | Secret to create (see secrets.mdx)
```

## `externalSecrets`
```yaml
externalSecrets: []              # struct[]  | ExternalSecret resources to create (see external-secrets.mdx)
```

## `extraObjects`
```yaml
extraObjects: []                 # any[]     | Extra raw Kubernetes objects to render (see extra-objects.mdx)
```
